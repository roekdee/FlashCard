...
function handleGetWords(e) {
  ...
  const autoGen = e.parameter.autoGen === '1'; // เพิ่ม parameter สำหรับควบคุม auto-generate
  
  const ss = SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID);
  const wordsSheet = ss.getSheetByName(CONFIG.SHEET_WORDS);
  
  // สร้าง ID, POS, Level อัตโนมัติเฉพาะเมื่อต้องการ (เพื่อหลีกเลี่ยง rate limit)
  if (autoGen) {
    autoGenerateData();
  }
  ...
  for (let i = 1; i < wordsData.length; i++) {
    const row = wordsData[i];
    const wordStr = row[colIndexes.word];
    if (!wordStr) continue; // skip empty rows
    
    // แยกคำจาก word string (เช่น "abandon v. B2" -> "abandon")
    const parsed = parseWordString(wordStr);
    const cleanWord = parsed ? parsed.word : wordStr;
    ...
    // ถ้ายังไม่มี id ให้ข้าม (แต่ปกติไม่น่าจะเกิด เพราะเรา gen ไปแล้ว)
    if (!word.id) continue;
...
function handleGetStats(e) {
  ...
  // นับจำนวนคำทั้งหมดใน sheet words
  const wordsData = wordsSheet.getDataRange().getValues();
  let totalWords = 0;
  for (let i = 1; i < wordsData.length; i++) {
    if (wordsData[i][1]) { // ถ้ามีคำในคอลัมน์ word
      totalWords++;
    }
  }
...
function handleSaveState(payload) {
  ...
  if (existingRow > 0) {
    ...
    if (learned !== undefined) {
      stateSheet.getRange(existingRow, colIndexes.learned + 1).setValue(learned);
    }
    if (hidden_forever !== undefined) {
      stateSheet.getRange(existingRow, colIndexes.hidden_forever + 1).setValue(hidden_forever);
    }
    stateSheet.getRange(existingRow, colIndexes.updated_at + 1).setValue(now);
  } else {
    // เพิ่มใหม่
    const newRow = [
      user_id,
...
function getExcludedWordIds(userId) {
  ...
  for (let i = 1; i < stateData.length; i++) {
    ...
    if (row[colIndexes.user_id] === userId) {
      ...
      const hidden = row[colIndexes.hidden_forever] === true || row[colIndexes.hidden_forever] === 'TRUE';
      if (learned || hidden) {
        excludedIds.add(row[colIndexes.word_id]);
      }
    }
  }
