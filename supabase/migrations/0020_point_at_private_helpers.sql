-- Repoint everything that used the public entitlement helpers at app_private.
...
begin
  ...
  select modes into v_modes from app_private.plan_of(v_uid);
...
set search_path = public, pg_temp
as $fn$
  with plan as (select * from app_private.plan_of(auth.uid())),
...
set search_path = public, pg_temp
as $fn$
  with m as (select * from app_private.user_metrics(auth.uid()))
...
begin
  if v_uid is null then raise exception 'not authenticated' using errcode = '28000'; end if;
  if not app_private.is_pro(v_uid) then
  ...
  with scored as (
    ...
    cross join lateral app_private.user_metrics(p.id) m
